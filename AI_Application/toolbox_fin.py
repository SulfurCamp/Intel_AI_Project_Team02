...
class SimpleFPS:
    ...
    def tick(self):
        ...
        if self._t_last is not None:
            ...
            self._fps = inst if self._fps is None else (
                self.alpha * self._fps + (1 - self.alpha) * inst
            )
        self._t_last = t
    def draw(self, frame, xy=(10, 24)):
        if self._fps is not None:
            cv2.putText(frame, f"FPS: {self._fps:5.1f}",
                        xy, cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255,255,255), 2, cv2.LINE_AA)
...
def visualize(output_queue: queue.Queue, cap: cv2.VideoCapture, save_stream_output: bool, output_dir: str,
    ...
    while True:
        ...
        if fps_tracker is not None:
            # 프로젝트의 FrameRateTracker가 increment()/get_fps() 패턴일 가능성
            try:
                fps_tracker.increment()
            except Exception:
                pass
            # 다양한 케이스 대비
            for attr in ("get_fps", "fps", "value"):
                if hasattr(fps_tracker, attr):
                    try:
                        fps_val = getattr(fps_tracker, attr)()
                    except TypeError:
                        fps_val = getattr(fps_tracker, attr)
                    except Exception:
                        pass
        ...
        if fps_val:
            cv2.putText(frame_with_detections, f"FPS: {fps_val:5.1f}",
                        (16, 32), cv2.FONT_HERSHEY_SIMPLEX, 0.7,
